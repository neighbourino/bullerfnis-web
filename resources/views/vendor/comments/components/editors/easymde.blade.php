<div>
    ...
    <div x-data="{ text: $wire.$entangle('{{ $model }}'), isEditing: @js($editing) }">
        <div x-data="markdownEditor_{{ $comment?->id ?? 'new' }}"
            @if ($comment) x-on:open-editor-{{ $comment->id }}.window="open"
                x-on:close-editor-{{ $comment->id }}.window="close" @endif>
            <div wire:ignore>
                <textarea x-ref="textarea" placeholder="{{ $placeholder }}"></textarea>
            </div>

            <div class="comments-form-editor-tip">
                Du kan bruge <a href="https://spatie.be/markdown" target="_blank"
                    rel="nofollow noopener noreferrer">Markdown</a>
            </div>
            ...
            <script>
                Alpine.data('markdownEditor_{{ $comment?->id ?? 'new' }}', () => ({
                    ...
                    open() {
                        ...
                        this.editor = MarkDownEditorUtils.initEasyMde(textarea,
                            {{ Spatie\LivewireComments\Support\Config::autoloadFontAwesome() ? 'true' : 'false' }});
                        ...
                        Livewire.on('mention-selected', (data, more) => {
                            ...
                            editor.codemirror.replaceRange('<span data-mention="' + data[0].id + '">' + data[0]
                                .display + '</span> ', {
                                    line: this.autoCompleteStart.line,
                                    ch: this.autoCompleteStart.ch - 1
                                }, cursorPosition);
                        ...
                        editor.codemirror.on('change', (codeMirror) => {
                            ...
                            if (this.showAutoComplete && this.mentionsEnabled) {
                                ...
                                $wire.dispatch('mention-autocomplete-search', {
                                    query
                                });
                            }
